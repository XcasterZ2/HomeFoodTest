...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const filename = `background_image_${Date.now()}${path.extname(fileField.filename || '')}`
    ...
    // Update restaurant background image in database     
    const updatedRestaurant = await prisma.restaurant.update({
      where: { restaurant_Id: restaurantId },
      data: {
        backgroundimage: publicUrlPath // เปลี่ยนจาก restaurantImage เป็น backgroundImage        
      }
    })
    ...
    console.error('Restaurant background image upload error:', error)
    throw createError({
      statusCode: 500,
      statusMessage: 'Error uploading background image'
    })
  } finally {
